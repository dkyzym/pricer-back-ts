      ...
      async (data: getItemResultsParams) => {
        ...
        if (supplier === 'profit') {
          ...
          try {
            ...
            if (!axios.isAxiosError(err)) {
              // => НЕ Axios-ошибка.
              userLogger.error('PATRIOT supplier: Non-Axios error occurred.', {
                message: (err as Error)?.message,
                stack: (err as Error)?.stack,
              });
            ...
            // 3. Если нет response, значит это сетевая или «неизвестная» ошибка
            if (!axiosError.response) {
              userLogger.error('PATRIOT supplier: Network or unknown error', {
                message: axiosError.message,
                stack: axiosError.stack,
              });
            ...
            // Логируем на уровне error, но только нужные детали
            userLogger.error(
              `PATRIOT supplier error: code=${errorCode}, message="${errorMessage}", httpStatus=${status}`,
              {
                stack: axiosError.stack,
                config: {
                  url: axiosError.config?.url,
