...
export const parseProfitApiResponse = (
  ...
  apiResponse.forEach((item) => {
    ...
    Object.entries(products).forEach(([productKey, product]) => {
      ...
      // Устанавливаем дату доставки: при наличии delivery_date из API — используем движок;
      // при отсутствии — рассчитываем через deadline (часы) как fallback.
      if (deliveryDateFromApi) {
        parsedItem.deliveryDate = deliveryDateFromApi;
        parsedItem.deliveryDate = calculateDeliveryDate(parsedItem, userLogger);
      } else {
        const hours = typeof deadline === 'number' && deadline > 0 ? deadline : 0;
        parsedItem.deliveryDate = DateTime.now()
          .setZone('UTC+3')
          .plus({ hours })
          .toFormat('yyyy-MM-dd');
      }
