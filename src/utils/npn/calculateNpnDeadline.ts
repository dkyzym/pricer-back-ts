...
const dayNamesForRegex = Object.keys(dayNameToIndex).join('|');
// Обновленное регулярное выражение: добавлена опциональная группа для захвата времени "после XX:XX"
const weeklyRuleRegex = new RegExp(
  `до\\s+(${dayNamesForRegex})[а-я.]*\\s.*?(\\d{1,2})[.:-](\\d{2}).*?доставка.*?(?:наш\\s+склад\\s+в|в)?\\s*(${dayNamesForRegex})[а-я.]*(?:\\s+после\\s+(\\d{1,2})[.:-](\\d{2}))?`
);
    ...
    try {
      ...
      if (weeklyMatch) {
        const [
          ...
          cutoffMinute,
          deliveryDayStr,
          deliveryHourStr, // Новая переменная: час доставки
          deliveryMinuteStr, // Новая переменная: минута доставки
        ] = weeklyMatch;
